	...
	struct fuse_file_info *fi)
{
	...
	sprintf(fullpaths[1], "%s%s", global_context.driveB, path);
     ...
     int read = 0;
     int tmp = 1;
     int set = (((int)offset) / 512);

	while(1) {
		fd = open(fullpaths[set & 1], O_RDONLY);
		if (fd == -1)
			return -errno;
		tmp = pread(fd, buf + read, 512, set / 2 * 512);
		if(read<size&&tmp==0)
			break;
		if (read == -1)
			read = -errno;
	...
	off_t offset, struct fuse_file_info *fi)
{
    ...
    int tmp = (((int)offset) / 512);
	int cal = 512 - (offset - tmp*512);    
	int cnt ;
	cnt= cal<size ? cal : size;

	fd = open(fullpaths[tmp & 1], O_WRONLY);
	tmp=tmp/2*512+512;
	if (fd == -1)
		return -errno;
	res = pwrite(fd, buf, cnt, tmp - cal);
	if (res == -1)
		res = -errno;
	close(fd);
